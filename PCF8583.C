...
#ifndef PCF8583_SDA
#define PCF8583_SDA  PIN_D1
#define PCF8583_SCL  PIN_D0
#endif
...
//real time
char	giay_ht=0,phut_ht=0,gio_ht=0;
char	thu_ht=0,ngay_ht=0,thang_ht=0,nam_ht=0;	
...
bcd_sec = bin2bcd(giay_ht);
bcd_min = bin2bcd(phut_ht);
bcd_hrs = bin2bcd(gio_ht);   
bcd_day = bin2bcd(ngay_ht) | (nam_ht << 6);
bcd_mon = bin2bcd(thang_ht) | (thu_ht << 5);
...
PCF8583_write_byte(PCF8583_YEAR_REG, nam_ht);
...
void PCF8583_read_datetime()
{
int8 CTRL_STATUS_REG;
...
i2c_write(PCF8583_CTRL_STATUS_REG);   // Start at seconds reg.
i2c_start();
i2c_write(PCF8583_READ_ADDRESS);

CTRL_STATUS_REG = i2c_read(1);     
i2c_read(1);     
...
if(CTRL_STATUS_REG!=PCF8583_START_COUNTING){
//	disable_interrupts(GLOBAL);
	i2c_start();
	i2c_write(PCF8583_WRITE_ADDRESS);
...
giay_ht = bcd2bin(bcd_sec);     
phut_ht = bcd2bin(bcd_min);     
gio_ht  = bcd2bin(bcd_hrs & 0x3F); 
ngay_ht = bcd2bin(bcd_day & 0x3F);
thang_ht= bcd2bin(bcd_mon & 0x1F);
thu_ht  = bcd_mon >> 5;
year_bits   = bcd_day >> 6;   
